...
from app01.models import User, Code, History ,OpLog
...
def fault_propogation(req):
    info = req.session.get("info")
    account = info["account"]
    code2 = Code.objects.filter(account=account)
    return render(req, 'fault_propgration.html', {'n1': code2, 'account': info['account']})

def history(req):
    info = req.session.get("info")
    account = info["account"]
    history =  History.objects.filter(account=account)
    return render(req, 'show_history.html', {'n1': history, 'account': info['account']})
...
def history_search(req, data):
    ...
    print(date_obj)
    print(2222)
    # 将日期对象转换为字符串，以便进行模糊查询
    date_str = date_obj.strftime('%Y-%m-%d')
    history = history.filter(account=account,time__contains=date_str)
    return render(req, 'search_history.html', {'n1': history, 'account': info['account']})
